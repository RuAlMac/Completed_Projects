...
/*UPDATES:
    Sleep Mode
    Radio Transceiver Acknowledgements
*/
...
void loop() {
  if (digitalRead(sleepSwitch) == LOW) { //If sleepSwitch is ON, sleep mode is OFF
    ...
    radio.read(&commandAcknowledge[0], sizeof(commandAcknowledge[0]));

    if (commandAcknowledge[0] == true) {
      digitalWrite(redLed, HIGH);
      delay(100);
      digitalWrite(redLed, LOW);
    }
    if (commandAcknowledge[0] == false) {
      Serial.println("NO");
    }
  }

  if (digitalRead(sleepSwitch) == HIGH) { //If sleepSwitch is OFF, sleep mode is ON
    if (toggleLights == 0) {
      lcd.noDisplay();
      lcd.noBacklight();
